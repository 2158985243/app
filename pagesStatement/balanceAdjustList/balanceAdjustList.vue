<template>
	<view class="balanceAdjustList">
	<u-navbar back-icon-color='#ffffff' title="余额调整" :background="background" title-color="#ffffff">
				<template slot="right">
							 ...
							 :autoPullUp="autoPullUp" :stopPullDown="stopPullDown" @onPullDown="handlePullDown" @onPullUp="handleLoadMore">
								<view class="cen">
									<view class="list" v-for="(itemList,indexList) in list[current]" :key="indexList" @click="toProcurementStatisticsDetails(itemList)">
										...
										<view class="right">
											<view class="money">
												<text :class="itemList.money>0? 'red':'green'">&yen;{{itemList.money}}</text>
												<text class="hui center">调整后余额：&yen;{{itemList.balance}}</text>
	...
	<script>
		...
		export default {
			...
			methods: {
				...
				showStrore() {
					uni.navigateTo({
						url: `/pagesStatement/pointAdjustList/pointQuery/pointQuery`
					})
				},
				// 店铺数组
				strored() {
				...
				async init(timeStar, timeEnd, keyword, store_ids, brand_id, goods_category_id) {
					...
					let res = await balanceAdjustList({
