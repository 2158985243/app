<template>
	<view class="u-upload" v-if="!disabled">
		<view v-if="showUploadList" class="u-list-item u-preview-wrap" v-for="(item, index) in lists" :key="index" :style="{
			...
			<image @tap.stop="doPreviewImage(item.url || item.path, index)" @longtap.stop="longtap(item,index)" class="u-preview-image"
			 v-if="!item.isImage" :src="item.url || item.path" :mode="imageMode"></image>
			<text class="main-img" v-if="index==0&&is_main">主</text>
		</view>
		...
		methods: {
			...
			longtap(item, index) {
				...
				uni.showModal({
					...
					success: async (res) => {
						if (res.confirm) {
							_this.lists.splice(0, 1, ..._this.lists.splice(index, 1, this.lists[0]));
							_this.$emit('long-tap', this.lists, index);
						} else {
